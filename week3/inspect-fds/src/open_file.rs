...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        ...
        let file_path = fs::read_link(link_path).ok()?;
        let file_path_str = file_path.to_str()?;

        let fdinfo_path = format!("/proc/{pid}/fdinfo/{fd}");
        let fdinfo_str = fs::read_to_string(fdinfo_path).ok()?;

        let name = Self::path_to_name(file_path_str);
        let cursor = Self::parse_cursor(&fdinfo_str)?;
        let access_mode = Self::parse_access_mode(&fdinfo_str)?;
